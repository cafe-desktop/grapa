		      ...
		      DialogData   *data)
{
	...
	recursive = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->include_subfold_checkbutton));
	follow_links = ! ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->exclude_symlinks));

	include_files = ctk_entry_get_text (GTK_ENTRY (data->include_files_entry));
	if (utf8_only_spaces (include_files))
		include_files = "*";

	exclude_files = ctk_entry_get_text (GTK_ENTRY (data->exclude_files_entry));
	if (utf8_only_spaces (exclude_files))
		exclude_files = NULL;

	exclude_folders = ctk_entry_get_text (GTK_ENTRY (data->exclude_folders_entry));
	       ...
	       void      *callback_data)
{
	...
	ctk_window_set_title (GTK_WINDOW (file_sel), _("Add a Folder"));
	...
	ctk_window_set_default_size (GTK_WINDOW (file_sel), 530, 510);

	data->choice = filechooser = ctk_file_chooser_widget_new (GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
	ctk_file_chooser_set_select_multiple (GTK_FILE_CHOOSER (filechooser), FALSE);
	...
	ctk_widget_set_halign (data->exclude_symlinks, GTK_ALIGN_START);
	ctk_widget_set_valign (data->exclude_symlinks, GTK_ALIGN_START);
	...
	ctk_label_set_xalign (GTK_LABEL (data->include_files_label), 0.0);
	ctk_label_set_mnemonic_widget (GTK_LABEL (data->include_files_label), data->include_files_entry);
	...
	ctk_label_set_xalign (GTK_LABEL (data->exclude_files_label), 0.0);
	ctk_label_set_mnemonic_widget (GTK_LABEL (data->exclude_files_label), data->exclude_files_entry);
	...
	ctk_label_set_xalign (GTK_LABEL (data->exclude_folders_label), 0.0);
	ctk_label_set_mnemonic_widget (GTK_LABEL (data->exclude_folders_label), data->exclude_folders_entry);
	...
	main_box = ctk_box_new (GTK_ORIENTATION_HORIZONTAL, 20);
	ctk_container_set_border_width (GTK_CONTAINER (main_box), 0);
 	ctk_widget_set_margin_start (GTK_WIDGET (main_box) , 6);

	content_area = ctk_dialog_get_content_area (GTK_DIALOG (file_sel));
	full_box = ctk_box_new (GTK_ORIENTATION_VERTICAL, 6);
 	ctk_widget_set_margin_top (GTK_WIDGET (full_box) , 2);
	ctk_container_set_border_width (GTK_CONTAINER (full_box), 0);
	ctk_box_pack_start (GTK_BOX (full_box), filechooser,
			    TRUE, TRUE, 0);
	ctk_box_pack_start (GTK_BOX (full_box), main_box, FALSE, FALSE, 0);
	...
	ctk_box_pack_start (GTK_BOX (vbox), data->include_subfold_checkbutton,
			    FALSE, FALSE, 0);

	ctk_box_pack_start (GTK_BOX (vbox), data->exclude_symlinks, FALSE, FALSE, 0);

	ctk_box_pack_start (GTK_BOX (vbox), data->add_if_newer_checkbutton,
			    FALSE, FALSE, 0);

	grid = ctk_grid_new ();
	ctk_grid_set_row_spacing (GTK_GRID (grid), 6);
	ctk_grid_set_column_spacing (GTK_GRID (grid), 6);
	ctk_box_pack_start (GTK_BOX (vbox), grid,
			    TRUE, TRUE, 0);
	...
	ctk_box_pack_start (GTK_BOX (vbox), data->load_button,
			    FALSE, FALSE, 0);
	ctk_box_pack_start (GTK_BOX (vbox), data->save_button,
			    FALSE, FALSE, 0);
	ctk_box_pack_start (GTK_BOX (vbox), data->clear_button,
			    FALSE, FALSE, 0);
			   ...
			   gboolean    no_symlinks)
{
	...
	if ((filename != NULL) && (strcmp (filename, base_dir) != 0))
		ctk_file_chooser_select_uri (GTK_FILE_CHOOSER (data->choice), filename);
	else
		grapa_file_chooser_set_current_folder_uri (GTK_FILE_CHOOSER (data->choice), base_dir);

	if (include_files != NULL)
		ctk_entry_set_text (GTK_ENTRY (data->include_files_entry), include_files);
	if (exclude_files != NULL)
		ctk_entry_set_text (GTK_ENTRY (data->exclude_files_entry), exclude_files);
	if (exclude_folders != NULL)
		ctk_entry_set_text (GTK_ENTRY (data->exclude_folders_entry), exclude_folders);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (data->add_if_newer_checkbutton), update);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (data->include_subfold_checkbutton), recursive);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (data->exclude_symlinks), no_symlinks);
}
			  ...
			  gboolean    *no_symlinks)
{
	...
	*update = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->add_if_newer_checkbutton));
	*recursive = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->include_subfold_checkbutton));
	*no_symlinks = ctk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->exclude_symlinks));
...
aod_update_option_list (LoadOptionsDialogData *aod_data)
{
	...
	while ((info = g_file_enumerator_next_file (file_enum, NULL, &err)) != NULL) {
		...
		ctk_list_store_append (GTK_LIST_STORE (aod_data->aod_model), &iter);
		ctk_list_store_set (GTK_LIST_STORE (aod_data->aod_model), &iter,
				    0, name,
				    1, display_name,
				    -1);
	       ...
	       LoadOptionsDialogData *aod_data)
{
	...
	ctk_list_store_remove (GTK_LIST_STORE (aod_data->aod_model), &iter);
		 ...
		 DialogData *data)
{
	...
	aod_data->aod_model = GTK_TREE_MODEL (ctk_list_store_new (2,
								  G_TYPE_STRING,
								  G_TYPE_STRING));
	ctk_tree_sortable_set_sort_column_id (GTK_TREE_SORTABLE (aod_data->aod_model),
					      0,
					      GTK_SORT_ASCENDING);
	ctk_tree_view_set_model (GTK_TREE_VIEW (aod_data->aod_treeview),
				 aod_data->aod_model);
	...
	ctk_tree_view_append_column (GTK_TREE_VIEW (aod_data->aod_treeview),
				     column);
	...
	ctk_window_set_transient_for (GTK_WINDOW (aod_data->dialog),
				      GTK_WINDOW (data->dialog));
	ctk_window_set_modal (GTK_WINDOW (aod_data->dialog), TRUE);
